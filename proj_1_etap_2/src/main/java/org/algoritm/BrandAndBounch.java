...
@Getter
@Setter
public class BrandAndBounch implements AlgInterface{
    ...
    private void TSPMatrix(int currentBound, int currentWeight, int level, int[] currentPath) {
        ...
        for (int i = 0; i < getNumOfVertexes(); i++) {
            ...
            if (getMatrix()[currentPath[level - 1]][i] != 0 && !visited[i]) {
                ...
                if (level == 1) currentBound -= ((firstMin(currentPath[level - 1]) + firstMin(i)) / 2);
                else currentBound -= ((secondMin(currentPath[level - 1]) + firstMin(i)) / 2);
                ...
                for (int j = 0; j <= level - 1; j++) getVisited()[currentPath[j]] = true;
    ...
    public String toString() {
        return getBestCost() + ";" + Arrays.toString(getBestTour()).replace(",", " -");
    }
}
